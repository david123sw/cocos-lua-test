...
extension.type_ali_share = "alipay_share" --支付宝分享
...
extension.type_xhr_progress = "URL_FETCH_PROGRESS" --下载进度
extension.replay_bgm = "replay_bgm" --重启BGM
...
--重启BGM
extension.replayBgm = function(call_back)
    extension.callBackHandler[extension.replay_bgm] = call_back
end

--小程序
extension.launchWXMiniProgram = function(miniProgramID, miniProgramPath, miniProgramType)
    ...
    miniProgramID = miniProgramID or "gh_f1091007dd58"
    miniProgramPath = miniProgramPath or "pages/index/index?open_id="..gt.playerData.openid.."&player_id="..gt.playerData.uid
    miniProgramType = miniProgramType or "2"
    local compVer = ""
    ...
    if compVer <= extension.getAppVersion() then
        if (gt.isIOSPlatform()) then
            extension.luaBridge.callStaticMethod(
                'AppController',
                'launchWXMiniProgram',
                {miniID=miniProgramID, miniPath=miniProgramPath, miniType=miniProgramType}
            )
        elseif (gt.isAndroidPlatform()) then
            extension.luaBridge.callStaticMethod(
                APIClass,
                'launchWXMiniProgram',
			    {miniProgramID, miniProgramPath, miniProgramType},
...
--设置语音上传地址
extension.initVoiceRecordAndPlayProcessAddr = function()
    local remoteServerAddr = ""
    local compVer = ""
    if gt.LoginServer.ip == "47.98.47.131" then
        remoteServerAddr = "http://ts.321mj.com:88"
        compVer = "1.0.1"
    else
        remoteServerAddr = "http://47.97.125.76:88"
        compVer = "1.0.1"
    end
    if compVer <= extension.getAppVersion() then
        if (gt.isIOSPlatform()) then
            extension.luaBridge.callStaticMethod(
                'yayavoice',
                'initVoiceRecordAndPlayProcessAddr',
                {addr=remoteServerAddr}
            )
        elseif (gt.isAndroidPlatform()) then
            extension.luaBridge.callStaticMethod(
                APIClass,
...
--是否模拟器---
extension.isEmulatorRun = function()
	local ok
	local ret = false
    if (gt.isAndroidPlatform()) then
...
--是否IphoneX---
extension.isAboveIphoneX = function()
	local ok
	local ret = false
    if (gt.isIOSPlatform()) then
        ok, ret = extension.luaBridge.callStaticMethod(
...
--获取电话簿---
extension.getAllPhoneContacts = function()
    local ok
	local contacts = ""
    if gt.isAndroidPlatform() then
        ok, contacts = extension.luaBridge.callStaticMethod(
            APIClass,
            'getAllPhoneContacts',
			nil,
            '()Ljava/lang/String;'
...
--开启屏幕旋转
extension.openOrientationChecker = function()
    local ok
	local ret = false
    if (gt.isIOSPlatform()) then
        --忽略
...
--是否安装ZFB---
extension.isZFBInstalled = function()
	local ok
	local ret = false
    if (gt.isIOSPlatform()) then
       ok, ret = extension.luaBridge.callStaticMethod(
...
extension.shareZFB = function(msgDic)
    if (gt.isIOSPlatform()) then
       extension.luaBridge.callStaticMethod(
            "AppController",
            'zfbShareMsg',
...
extension._getLocationHandler = function(data)
    if(gt.isAndroidPlatform()) then
        ...
        if (tonumber(status) == 1) then
            local decodedGP = data.code
            decodedGP = string.split(decodedGP, "#")
            gt.location = {lat=decodedGP[1], lng=decodedGP[2], addr=decodedGP[3]}
        else
            gt.location = ""
        end
    elseif (gt.isIOSPlatform()) then
        ...
        if latStr == nil or lonStr == nil or addStr == nil then
            ...
            gt.location = {lat=latStr, lng=lonStr, addr=addStr}
        end
    else
        gt.location = ""
    end
...
--    gt.log("extension._getLocationHandler,curScene:getName() = "..curScene:getName())
    if curScene and curScene.sendNewGpMessage then
        curScene:sendNewGpMessage()
    end
end
...
extension.AndroidWXPay = function(orderInfo)
	local function payHandler(jsonData)
        ...
        if(status == 1) then
            gt.log("微信支付成功")
            require("app/views/NoticeTips"):create(gt.getLocationString("LTKey_0007"), "微信支付成功", nil, nil, true)
        else
            gt.log("微信支付取消 errorCode", jsonData.code)
            require("app/views/NoticeTips"):create(gt.getLocationString("LTKey_0007"), "微信支付取消[Code="..jsonData.code.."]", nil, nil, true)
        end
	end
